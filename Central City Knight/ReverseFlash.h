#ifndef REVERSEFLASH_H_INCLUDED
#define REVERSEFLASH_H_INCLUDED
...
void drawHead()
{
   ...
   /* glPushMatrix();
	...
	for(int i=0;i<200;i++)
	{
		...
		float r=0.2;
		float x = r * cos(A);
		float y = r * sin(A);float drawCurve(float p1, float p2, float p3,float p4, float t)
{
    float pf=0;
    float s=1-t;
    pf=p1*pow(s,3) + 3*p2*pow(s,2)*t+ 3*s*pow(t,2)*p3 + pow(t,3)*p4;
